...
function FullKnowledgeGraph() {
    ...
    return (_jsxs("div", { className: "bg-card border border-[var(--color-border-light)] shadow-sm overflow-hidden", children: [_jsxs("div", { className: "flex items-center justify-between px-5 py-3 border-b border-[var(--color-border-light)]", children: [_jsxs("div", { className: "flex items-center gap-2", children: [_jsx(Network, { className: "size-5 text-accent" }), _jsx("span", { className: "text-2xl font-semibold text-text", children: "Full Knowledge Graph" })] }), _jsxs("span", { className: "text-xl text-muted", children: [nodes.length, " nodes \u00B7 ", links.length, " edges"] })] }), _jsx("div", { ref: containerRef, style: { position: 'relative' }, children: _jsx("canvas", { ref: canvasRef, width: 900, height: 600, style: { width: '100%', height: 600, cursor: 'grab', background: '#0d1117' }, onMouseDown: handleCanvasMouseDown, onMouseMove: handleCanvasMouseMove, onMouseUp: handleCanvasMouseUp, onMouseLeave: handleCanvasMouseUp }) }), _jsx("div", { className: "px-4 py-3 border-t border-[var(--color-border-light)] flex flex-wrap gap-3", children: typeCounts.slice(0, 12).map(([type, count]) => (_jsxs("div", { className: "flex items-center gap-1.5 text-lg uppercase text-muted", children: [_jsx("div", { className: "w-2.5 h-2.5 rounded-full", style: { backgroundColor: getNodeColor(type) } }), _jsx("span", { children: type }), _jsxs("span", { className: "text-xl opacity-60 font-black", children: ["(", count, ")"] })] }, type))) })] }));
}
const EDGE_COLORS = {
    depends_on: '#f97316',
    tests: '#3b82f6',
...
export function GraphGlobal() {
    ...
    return (_jsxs("div", { className: "space-y-6", children: [_jsxs("div", { children: [_jsx("h1", { className: "text-6xl font-bold text-text", children: "Knowledge Graph" }), _jsx("p", { className: "text-2xl text-muted mt-1", children: "Explore project knowledge graphs and inter-project dependencies" })] }), projects.length === 0 ? (_jsxs("div", { className: "bg-card rounded border border-[var(--color-border-light)] shadow-sm p-12 text-center", children: [_jsx(FolderKanban, { className: "w-10 h-10 text-[var(--color-text-light)] mx-auto mb-3" }), _jsx("p", { className: "text-sm text-muted", children: "No projects yet. Create a project to get started." }), _jsx(Link, { to: "/new-project", className: "inline-flex items-center gap-2 mt-4 px-4 py-2 rounded-[var(--radius-md)] bg-accent text-white text-sm font-medium hover:bg-[var(--color-accent-hover)] transition-colors", children: "Create Project" })] })) : (_jsxs(_Fragment, { children: [_jsxs("div", { children: [_jsx("h2", { className: "text-4xl font-semibold text-text mb-3", children: "Project Knowledge Graphs" }), _jsx("div", { className: "grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4", children: projects.map((p) => (_jsxs(Link, { to: `/graph/${p.id}`, className: "group bg-card border border-[var(--color-border-light)] shadow-sm p-5 hover:shadow-md hover:border-info/30 transition-all flex items-center justify-between", children: [_jsxs("div", { className: "flex items-center gap-3", children: [_jsx("div", { className: "w-10 h-10 rounded-full ring-2 ring-[var(--color-info)] bg-[color-mix(in_srgb,var(--color-info)_10%,transparent)] flex items-center justify-center flex-shrink-0 group-hover:bg-[color-mix(in_srgb,var(--color-info)_15%,transparent)] transition-colors", children: _jsx(GitBranch, { className: "w-5 h-5 text-info" }) }), _jsxs("div", { children: [_jsx("h3", { className: "text-2xl font-semibold text-text group-hover:text-info transition-colors", children: p.name }), p.description && (_jsx("p", { className: "text-md text-muted mt-0.5 line-clamp-2", children: p.description }))] })] }), _jsx(ArrowRight, { className: "w-4 h-4 text-[var(--color-text-light)] group-hover:text-info transition-colors" })] }, p.id))) })] }), _jsxs("div", { children: [_jsx("h2", { className: "text-4xl mt-5 font-semibold text-text mb-3", children: "Project Dependencies" }), _jsxs("div", { ref: containerRef, className: "bg-card rounded-[var(--radius-lg)] border border-[var(--color-border-light)] shadow-sm overflow-hidden relative", children: [!hasLinks && (_jsxs("div", { className: "absolute top-4 left-1/2 -translate-x-1/2 z-10 px-4 py-2 rounded-[var(--radius-md)] bg-[var(--color-hover)] border border-[var(--color-border-light)] text-xs text-muted flex items-center gap-2", children: [_jsx(Link2, { className: "w-3.5 h-3.5" }), "Link projects to see dependencies"] })), _jsxs("svg", { ref: svgRef, width: "100%", height: 500, className: "cursor-grab active:cursor-grabbing", onMouseDown: handleSvgMouseDown, onMouseMove: handleMouseMove, onMouseUp: handleMouseUp, onMouseLeave: handleMouseUp, children: [_jsx("defs", { children: Object.entries(EDGE_COLORS).map(([type, color]) => (_jsx("marker", { id: `arrow-${type}`, viewBox: "0 0 10 6", refX: 38, refY: 3, markerWidth: 8, markerHeight: 6, orient: "auto", children: _jsx("path", { d: "M0,0 L10,3 L0,6 Z", fill: color }) }, type))) }), _jsxs("g", { transform: `translate(${panOffset.x},${panOffset.y})`, children: [edges.map((edge) => {
                                                        const source = edge.source;
                                                        const target = edge.target;
                                                        if (source.x == null || target.x == null)
                                                            return null;
                                                        const color = EDGE_COLORS[edge.linkType] ?? EDGE_COLORS.related;
                                                        return (_jsx("line", { x1: source.x, y1: source.y, x2: target.x, y2: target.y, stroke: color, strokeWidth: 2, strokeOpacity: 0.3, markerEnd: `url(#arrow-${edge.linkType})` }, edge.id));
                                                    }), nodes.map((node) => {
