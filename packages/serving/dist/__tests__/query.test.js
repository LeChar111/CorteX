...
describe('POST /api/query', () => {
    it('returns 200 with response for a valid query', async () => {
        vi.stubGlobal('fetch', makeFetchMock({ ok: true, json: { response: 'The answer is 42.' } }));
        const app = makeApp();
    ...
    it('uses the provided mode', async () => {
        ...
        expect(JSON.parse(init.body).mode).toBe('local');
    });
    ...
    it('returns 500 when LightRAG is unavailable', async () => {
        vi.stubGlobal('fetch', makeFetchMock({ ok: false, status: 503, text: 'Service Unavailable' }));
        const app = makeApp();
        const res = await app.request('/api/query', {
            method: 'POST',
            headers: authHeaders(),
