...
import { z } from 'zod';
import { getDb, getProjectById, getGraphForProject, getGraphFull, getCommunitiesForProject, } from '@cortex/db';
...
analysisRoutes.post('/analysis/impact', async (c) => {
    ...
    const hop1Edges = allEdges.filter((e) => fileNodeIds.has(String(e.sourceNodeId ?? e.source_node_id ?? '')) ||
    ...
    const hop2Edges = allEdges.filter((e) => hop1Ids.has(String(e.sourceNodeId ?? e.source_node_id ?? '')) ||
...
analysisRoutes.get('/analysis/dead-code', async (c) => {
    ...
    const targetIds = new Set(graph.edges.map((e) => String(e.targetNodeId ?? e.target_node_id ?? '')));
    const unreferenced = graph.nodes.filter((n) => !targetIds.has(String(n.id ?? '')));
    ...
    const deadCode = unreferenced.filter((n) => {
        ...
        if (nonCodeTypes.has(entityType))
            return false;
...
analysisRoutes.get('/analysis/health-score', async (c) => {
    ...
    const projectNodeIds = new Set(graph.nodes.map((n) => String(n.id ?? '')));
    const totalNodes = graph.nodes.length;
    const targetIds = new Set(graph.edges.map((e) => String(e.targetNodeId ?? e.target_node_id ?? '')));
    const orphanCount = graph.nodes.filter((n) => !targetIds.has(String(n.id ?? ''))).length;
    ...
    const graphCoverage = totalNodes > 0
        ...
        : 0;
    const score = Math.max(0, Math.min(100, graphCoverage * 0.4 +
        (100 - orphanRatio) * 0.3 +
...
analysisRoutes.get('/analysis/surprising-connections', async (c) => {
    ...
    const surprisingConnections = communities.flatMap((comm) => {
        const connections = (comm.surprisingConnections ?? []);
