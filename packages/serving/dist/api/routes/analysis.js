...
async function getProjectKeywords(projectId) {
    ...
    for (const r of repos) {
        ...
        if (r.slug && r.slug !== r.name)
            keywords.push(r.slug.toLowerCase());
    }
    return keywords;
}
// Helper: check if a node belongs to a project (by matching keywords in description/source)
function nodeMatchesProject(n, keywords) {
    const p = nodeProps(n);
...
analysisRoutes.post('/analysis/impact', async (c) => {
    ...
    const hop1Edges = allEdges.filter((e) => fileNodeIds.has(String(e.source ?? '')) || fileNodeIds.has(String(e.target ?? '')));
...
analysisRoutes.get('/analysis/dead-code', async (c) => {
    ...
    const unreferenced = projectNodes.filter((n) => !targetIds.has(String(n.id ?? '')));
...
analysisRoutes.get('/analysis/health-score', async (c) => {
    ...
    const projectEdges = allEdges.filter((e) => projectNodeIds.has(String(e.source ?? '')) || projectNodeIds.has(String(e.target ?? '')));
    const totalNodes = projectNodes.length;
    const targetIds = new Set(projectEdges.map((e) => String(e.target ?? '')));
    const orphanCount = projectNodes.filter((n) => !targetIds.has(String(n.id ?? ''))).length;
