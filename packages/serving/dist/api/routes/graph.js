...
graphRoutes.get('/graph/summary', async (c) => {
    const db = getDb();
    const projectId = c.req.query('projectId');
    const mode = (c.req.query('mode') ?? 'hubs');
    ...
    try {
        if (mode === 'hubs') {
          ...
          `;
            ...
            const topNodes = (topNodesResult.rows ?? topNodesResult ?? []);
      ...
      `;
        ...
        if (mode === 'communities') {
          ...
          `;
            ...
            const samples = (samplesResult.rows ?? samplesResult ?? []);
            if (samples.length === 0) {
                return c.json({ nodes: [], edges: [], mode, total: 0 });
            }
            // Sort representatives by community size (biggest first)
            samples.sort((a, b) => Number(b.community_size ?? 0) - Number(a.community_size ?? 0));
            ...
            const repIds = samples.map((r) => String(r.id));
            const repIdsSql = sql.raw(`ARRAY[${repIds.map(id => `'${id.replace(/'/g, "''")}'`).join(',')}]::varchar[]`);
            const communityIds = samples.map((r) => r.community_id);
            const commIdsSql = sql.raw(`ARRAY[${communityIds.map((id) => `'${String(id).replace(/'/g, "''")}'`).join(',')}]::varchar[]`);
          ...
          `;
            ...
            try {
                ...
                aggregatedEdges = (aggResult.rows ?? aggResult ?? []);
            }
            catch (e) {
                console.error('[graph/summary] aggregated edges error:', e);
            }
            const pairMap = new Map();
            const addPair = (a, b, w, relation) => {
                if (a === b)
                    return;
            ...
            const byDir = new Map(); // dir -> rep ids
            ...
            for (const n of samples) {
                ...
                const anchor = siblings.find(id => id !== rid && connected.has(id)) ??
...
graphRoutes.get('/graph/community', async (c) => {
    ...
    try {
        ...
        const nodes = (nodesResult.rows ?? nodesResult ?? []);
        if (nodes.length === 0) {
            return c.json({ nodes: [], edges: [], total: 0 });
        }
        const ids = nodes.map((r) => String(r.id));
    ...
    `;
        ...
        const totalRows = (totalCountRow.rows ?? totalCountRow ?? []);
