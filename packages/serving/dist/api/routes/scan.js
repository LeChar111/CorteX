...
scanRouter.put('/scan/:jobId/pause', async (c) => {
    ...
    await updateScanJob(db, job.id, { status: 'paused', error: 'Paused by user' });
...
scanRouter.put('/scan/:jobId/resume', async (c) => {
    ...
    await updateScanJob(db, job.id, { status: 'failed', error: `Superseded by ${newJob.id}` });
...
scanRouter.get('/scan/queue', async (c) => {
    ...
    for (const job of runningJobs) {
        ...
        const hasNewerTerminal = allJobs.some((other) => other.id !== job.id &&
...
scanRouter.put('/scan/batch/pause', async (c) => {
    ...
    for (const jobId of jobIds) {
        ...
        if (!job || (job.status !== 'running' && job.status !== 'queued'))
            continue;
        await updateScanJob(db, jobId, { status: 'paused', error: 'Paused by user' });
        paused.push(jobId);
    }
