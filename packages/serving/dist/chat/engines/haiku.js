...
async function streamViaCli(prompt, writer) {
    return new Promise((resolve, reject) => {
        ...
        const proc = spawn('claude', ['--print', '--model', CLI_MODEL, '--max-turns', '5', prompt], { stdio: ['ignore', 'pipe', 'pipe'] });
        let stderr = '';
...
async function streamViaApi(input, systemPrompt, writer) {
    const messages = [
        ...input.history.map((m) => ({
            role: m.role,
            content: m.content,
