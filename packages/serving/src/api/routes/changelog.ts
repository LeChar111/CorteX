...
changelogRoutes.get('/changelog', async (c) => {
  ...
  const totalFiles = recentJobs.reduce((sum, j) => sum + ((j.stats as Record<string, number>)?.filesProcessed ?? 0), 0);
  const totalEntities = recentJobs.reduce((sum, j) => sum + ((j.stats as Record<string, number>)?.entitiesExtracted ?? 0), 0);
