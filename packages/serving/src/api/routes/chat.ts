...
import type { ChatMessage, ResolvedMode, SSEWriter } from '../../chat/types.js';
...
chatRouter.post('/chat', async (c) => {
  ...
  const history: ChatMessage[] = flat.slice(0, -1).slice(-20);

  const resolvedMode: ResolvedMode =
    mode === 'auto' ? routeQuery(userMessage, !!projectId) : mode;
