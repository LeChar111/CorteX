 ...
 *   MY_KEY=secret_value
 */
async function readCredFile(): Promise<CredEntry[]> {
  let raw: string;
  try {
...
async function writeCredFile(entries: CredEntry[]): Promise<void> {
  const lines: string[] = [
...
credentialsRouter.get('/credentials/:id/reveal', async (c) => {
  ...
  if (!entry) return c.json({ error: 'Credential not found' }, 404);
  return c.json(entry);
});
...
credentialsRouter.put('/credentials/:id', async (c) => {
  ...
  if (idx === -1) return c.json({ error: 'Credential not found' }, 404);
  ...
  entries[idx] = { ...entries[idx]!, ...parsed.data } as CredEntry;
  await writeCredFile(entries);
