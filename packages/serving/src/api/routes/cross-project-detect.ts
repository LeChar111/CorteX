...
crossProjectDetectRoutes.post('/analysis/detect-links', async (c) => {
  ...
  const srcLabels = new Map<string, (typeof srcGraph.nodes)[0]>();
  ...
  const sharedEntities: Array<{
    name: string;
    sourceNode: { id: string; label: string; type: string; file: string | null };
    targetNode: { id: string; label: string; type: string; file: string | null };
    matchType: string;
  }> = [];
  ...
  const getImportNames = (nodes: typeof srcGraph.nodes) => {
  ...
  if (overlapRatio > 0.3) suggestedLinkType = 'shares_lib';
  else if (sharedImports.length > 5) suggestedLinkType = 'depends_on';
