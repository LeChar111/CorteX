...
documentsRouter.get('/documents', async (c) => {
  ...
  const total = (statusCounts as Record<string, number>).all ?? docs.documents.length;
...
documentsRouter.get('/documents/pipeline', async (c) => {
  ...
  const [pipeline, counts, graphLabels] = await Promise.all([
    ...
    client.getStatusCounts().catch(() => ({} as Record<string, number>)),
    graphLabelsFetch as Promise<string[]>,
  ]);
