...
documentsRouter.get('/documents/pipeline', async (c) => {
  ...
  for (const evt of recentEvents) {
    ...
    if (evt.type === 'scan.completed') {
      ...
      historyMessages.push(
        `[${ts}] Scan completed — ${stats.nodesImported ?? 0} nodes, ${stats.edgesImported ?? 0} edges imported`,
      );
    } else if (evt.type === 'scan.failed') {
      historyMessages.push(`[${ts}] Scan failed — ${(payload as any)?.error ?? 'unknown error'}`);
    } else {
  ...
  const jobName = runningJob
    ? `Scanning (phase: ${(runningJob.stats as any)?.phase ?? 'unknown'})`
    : '';

  if (runningJob) {
    const stats = runningJob.stats as Record<string, unknown> | null;
    historyMessages.unshift(
      `[running] Phase: ${stats?.phase ?? 'unknown'} — ${stats?.filesProcessed ?? 0}/${stats?.total ?? '?'} files processed`,
    );
  }
