...
graphOpsRoutes.delete('/graph/relation', async (c) => {
  ...
  try {
    await db.delete(graphEdges).where(
      and(
        eq(graphEdges.sourceNodeId, nameToId(source)),
        eq(graphEdges.targetNodeId, nameToId(target)),
      ),
    );
...
graphOpsRoutes.get('/graph/cross-project', async (c) => {
  ...
  if (entityName) {
    ...
    for (const n of allNodes) {
      ...
      if (label.includes(nameLower)) seedIds.add(String(n.id));
    }
    ...
    const filteredEdges = allEdges.filter((e) =>
      visited.has(String(e.source ?? '')) && visited.has(String(e.target ?? '')),
    );

    const taggedNodes = filteredNodes as Array<Record<string, unknown> & { _project: string }>;
  ...
  const multiProjectEdges = allEdges.filter((e) =>
