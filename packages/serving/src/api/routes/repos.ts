...
reposRouter.get('/repos/ssh-keys', async (c) => {
  ...
  try {
    ...
    const keys: { name: string; path: string; type: 'private' | 'public' }[] = [];
    for (const f of files) {
      if (['config', 'known_hosts', 'known_hosts.old', 'known_hosts_old', 'authorized_keys', 'environment'].includes(f)) continue;
      const fullPath = join(sshDir, f);
      const s = await stat(fullPath);
      if (!s.isFile() || s.size > 100_000) continue;
      if (f.endsWith('.pub')) {
...
async function loadCredEnv(): Promise<Record<string, string>> {
  ...
  try {
    ...
    for (const line of raw.split('\n')) {
      ...
      if (eq > 0) entries[trimmed.slice(0, eq)] = trimmed.slice(eq + 1);
    }
    return entries;
  } catch {
    return {};
...
async function fetchRemoteBranches(cloneUrl: string, provider: string): Promise<string[]> {
  const creds = await loadCredEnv();

  if (provider === 'bitbucket') {
    // Extract org/repo from clone URL
    const match = cloneUrl.match(/bitbucket\.org\/([^/]+)\/([^/.]+)/);
    if (!match) throw new Error('Cannot parse Bitbucket URL');
    ...
    let url: string | null = `https://api.bitbucket.org/2.0/repositories/${workspace}/${repoSlug}/refs/branches?pagelen=100`;

    while (url) {
      const res = await fetch(url, { headers });
      if (!res.ok) throw new Error(`Bitbucket API error: ${res.status}`);
      const data = await res.json() as { values: { name: string }[]; next?: string };
      branches.push(...data.values.map((b: { name: string }) => b.name));
      url = data.next ?? null;
    }
  ...
  if (provider === 'github') {
    ...
    return data.map((b: { name: string }) => b.name).sort();
  }
