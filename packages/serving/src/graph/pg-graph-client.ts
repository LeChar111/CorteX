...
export class PgGraphClient {
  ...
  async getEntityGraph(entityName: string, depth = 2) {
    const db = getDb();
  ...
  async searchEntities(query: string, projectId?: string, limit = 50) {
    const db = getDb();
    const nodes = await searchGraphNodes(db, query, projectId, limit);
    return nodes.map(nodeToApi);
  }

  async getCommunities(projectId: string) {
    const db = getDb();
...
function nodeToApi(node: Record<string, unknown>) {
  return {
    id: node.id,
    label: node.label,
    type: node.type,
...
function edgeToApi(edge: Record<string, unknown>) {
  return {
...
// Singleton
let instance: PgGraphClient | null = null;
export function getPgGraphClient(): PgGraphClient {
  if (!instance) instance = new PgGraphClient();
  return instance;
}
